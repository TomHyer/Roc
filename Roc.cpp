...
#ifdef _MSC_VER
typedef unsigned long bitscan_t;
#else
typedef unsigned bitscan_t;
#include <cmath>
#include <algorithm>
#endif
...
constexpr int NO_INFO = -9999;
static int theLastInfo = NO_INFO;
...
struct Provenance
{
	struct Entry
	{
		...
		sint64 src;
		int info;
		Entry(int l, const array<double, 4>& c, sint64 s) : line(l), contrib(c), src(s), info(theLastInfo) {}
	};
	std::vector<Entry> vals;
	Provenance(const std::vector<Entry>& vals_in) : vals(vals_in) {}
};

shared_ptr<Provenance> NoProvenance() 
{ 
	theLastInfo = NO_INFO;
	return make_shared<Provenance>(std::vector<Provenance::Entry>()); 
}
...
INLINE sint64 Pack4(sint16 op, sint16 mid, sint16 eg, sint16 asym)
{
...
static const int DrawCapConstant = 110 * CP_EVAL;
...
#else
#ifdef TUNER_DATA
void Do_IncV(packed_t& dst, sint64 inc, int line, bool fake = false)
{
	if (inc == 0)
		return;
	if (!fake)
...
void Do_Notice(packed_t& dst, int val, int line)
{
	theLastInfo = val;
}

#define IncV(var, x) Do_IncV(var, me ? -(x): x, __LINE__)
#define FakeV(var, x) Do_IncV(var, me ? -(x) : x, __LINE__, true)
...
template<class POP> score_t score_from_material(sint16 val)
{
	...
	{
		// MatLinear
		IncV(retval, 100 * p[me]);
		IncV(retval, 300 * n[me]);
		IncV(retval, 300 * b[me]);
		IncV(retval, 500 * r[me]);
		IncV(retval, 900 * q[me]);
		IncV(retval, b[me] > 1 ? 50 : 0);
		// MatQuadMe
		IncV(retval, 10 * p[me] * p[me]);
		IncV(retval, 30 * p[me] * n[me]);
		IncV(retval, 30 * p[me] * b[me]);
		IncV(retval, 30 * p[me] * r[me]);
		IncV(retval, 50 * p[me] * q[me]);
		IncV(retval, 90 * n[me] * n[me]);
		IncV(retval, 90 * n[me] * b[me]);
		IncV(retval, 90 * n[me] * r[me]);
		IncV(retval, 150 * n[me] * q[me]);
		IncV(retval, 90 * b[me] * b[me]);
		IncV(retval, 90 * b[me] * r[me]);
		IncV(retval, 150 * b[me] * q[me]);
		IncV(retval, 90 * r[me] * r[me]);
		IncV(retval, 150 * r[me] * q[me]);
		// MatQuadOpp
		IncV(retval, 30 * p[me] * n[opp]);
		IncV(retval, 30 * p[me] * b[opp]);
		IncV(retval, 30 * p[me] * r[opp]);
		IncV(retval, 50 * p[me] * q[opp]);
		IncV(retval, 90 * n[me] * b[opp]);
		IncV(retval, 90 * n[me] * r[opp]);
		IncV(retval, 150 * n[me] * q[opp]);
		IncV(retval, 90 * b[me] * r[opp]);
		IncV(retval, 150 * b[me] * q[opp]);
		IncV(retval, 150 * r[me] * q[opp]);
		// BishopPairQuad
		if (b[me] > 1)
		{
			IncV(retval, 30 * p[me]);
			IncV(retval, 90 * n[me]);
			IncV(retval, 90 * r[me]);
			IncV(retval, 150 * q[me]);
			IncV(retval, 30 * p[opp]);
			IncV(retval, 90 * n[opp]);
			IncV(retval, 90 * b[opp]);
			IncV(retval, 90 * r[opp]);
			IncV(retval, 150 * q[opp]);
		}
	}
	retval.val = val;
	return retval;
...
array<GPawnEntry, pawn_hash_size> PawnHash;
...
constexpr array<int, 6> MatLinear = { 26, -14, -12, 84, -20, -10 };
...
constexpr array<int, 9> BishopPairQuad = { // tuner: type=array, var=1000, active=0
	-80, 139, 63, 237, -103, -90, -201, 54, -241
};
constexpr array<int, 6> MatClosed = { 13, 37, 11, -13, -7, 24 };
...
constexpr array<int, 12> MobCoeffsKnight = { 1231, 753, 518, -340, 1832, 1170, 524, 761, 241, 156, -61, 164 };
constexpr array<int, 12> MobCoeffsBishop = { 1300, 678, 503, -220, 2106, 1613, 1443, -336, -243, 271, 109, 501 };
constexpr array<int, 12> MobCoeffsRook = { 1038, 840, 675, 35, -542, 323, 1189, 354, 84, 26, -4, -3 };
constexpr array<int, 12> MobCoeffsQueen = { 472, 838, 1169, 96, 2157, 529, -1054, -210, 48, -7, -7, -1 };
...
array<sint64, 8> PasserGeneral, PasserBlocked, PasserFree, PasserSupported, PasserProtected, PasserConnected, PasserOutside, PasserCandidate, PasserClear;
...
constexpr array<int, 48> Tactical = {  // tuner: type=array, var=51, active=0
	4, 1, 3, 3,
	53, 20, 69, -7,
	6, 11, 37, 22,
	29, 29, 66, 10,
...
enum { BishopOutpostNoMinor };
constexpr array<int, 4> BishopSpecial = { // tuner: type=array, var=20, active=0
	72, 73, 26, 15
};
...
constexpr array<uint16, 16> KingAttackScale = { 0, 1, 2, 3, 4, 6, 11, 17, 24, 31, 39, 47, 46, 65, 65, 65 };
constexpr array<int, 4> KingCenterScale = { 57, 56, 67, 64 };
	 ...
	 T_* mob)
{
	...
	{
		double val = 1.12 * pow(pop, 0.95) * (coeffs[phase] - coeffs[phase + 8]) + log(0.90 + pop) * coeffs[phase + 4];
...
void init_eval()
{
	...
	{
		int im2 = Max(i - 2, 0);
		auto attdef = [&](int k) { return PasserAttDefQuad[k] * im2*im2 + PasserAttDefLinear[k] * im2; };
...
void calc_material(int index)
{
	...
	Material[index].imbalance = 6 * (queens[White] - queens[Black]) + 3 * (rooks[White] - rooks[Black]) + 2 * (minor[White] - minor[Black]);
...
template <bool me, class POP> INLINE void eval_pawns(GPawnEntry* PawnEntry, GPawnEvalInfo& PEI)
{
	...
	for (uint64 u = Pawn(me); T(u); u ^= b)
	{
		...
		else
		{
			...
			{
				IncV(PEI.score, Ca4(PawnSpecial, PawnChainLinear) * (rrank - 4));
...
template <bool me, class POP> INLINE void eval_queens(GEvalInfo& EI)
{
	...
	for (u = Queen(me); T(u); u ^= b)
	{
		...
		FakeV(EI.score, (64 * pop(control & KingLocus[EI.king[opp]]) - N_LOCUS * pop(control)) * Pack4(1, 1, 1, 1));
...
template <bool me, class POP> INLINE void eval_passer(GEvalInfo& EI)
{
	...
	for (uint8 u = EI.PawnEntry->passer[me]; T(u); u &= (u - 1))
	{
		...
		//if (EI.material)
		//{
		//	int deficit = (me ? 1 : -1) * EI.material->imbalance;
		//	if (rank <= deficit) continue;
		//}
		...
		if (!(way & Piece(opp)))
		{
			DecV(EI.score, PasserClear[rank]);
...
template <bool me, class POP> INLINE void eval_pieces(GEvalInfo& EI)
{
	...
	if (uint64 weakThreat = Current->dbl_att[opp] & ~(Current->att[me] | threat) & Piece(me))
		DecV(EI.score, Ca4(Tactical, TacticalWeakThreat));
...
template<class POP> void evaluation()
{
	...
	{
		int me = White;
		FakeV(EI.score, 100 * (pop(Pawn(White)) - pop(Pawn(Black))) * Pack4(0, 0, 0, 1));
		FakeV(EI.score, 300 * (pop(Knight(White)) - pop(Knight(Black))) * Pack4(0, 0, 0, 1));
		FakeV(EI.score, 300 * (pop(Bishop(White)) - pop(Bishop(Black))) * Pack4(0, 0, 0, 1));
		FakeV(EI.score, 500 * (pop(Rook(White)) - pop(Rook(Black))) * Pack4(0, 0, 0, 1));
		FakeV(EI.score, 900 * (pop(Queen(White)) - pop(Queen(Black))) * Pack4(0, 0, 0, 1));
		FakeV(EI.score, 900 * (Multiple(Bishop(White)) - Multiple(Bishop(Black))) * Pack4(0, 0, 0, 1));
	}
	...
	if (mat)
	{
		...
		if (Current->score > 0)
		{
			...
			if (uint64 advanced = Current->passer & Pawn(Black) & (Line[5] | Line[6]))
			{
				if (Multiple(advanced) || (advanced & Line[6]))
					EI.mul -= EI.mul / 3;
				else
					EI.mul -= EI.mul / 7;
			}
		...
		else if (Current->score < 0)
		{
			...
			if (uint64 advanced = Current->passer & Pawn(White) & (Line[2] | Line[1]))
			{
				if (Multiple(advanced) || (advanced & Line[1]))
					EI.mul -= EI.mul / 3;
				else
					EI.mul -= EI.mul / 7;
			}
...
template <bool me> int* gen_captures(int* list)
{
	...
	if (Current->ep_square)
		for (v = PAtt[opp][Current->ep_square] & Pawn(me); T(v); Cut(v)) 
			list = AddMove(list, lsb(v), Current->ep_square, FlagEP, MvvLva[IPawn[me]][IPawn[opp]] + bonus(lsb(v)));
	...
	if (T(Current->att[me] & Current->mask))
	{
		for (v = ShiftW<opp>(Current->mask) & Pawn(me) & (~OwnLine(me, 6)); T(v); Cut(v))
		{
			int from = lsb(v), to = lsb(v) + PushE[me];
			list = AddCaptureP(list, IPawn[me], from, to, 0, bonus(to));
		}
		for (v = ShiftE<opp>(Current->mask) & Pawn(me) & (~OwnLine(me, 6)); T(v); Cut(v)) 
		{
			int from = lsb(v), to = lsb(v) + PushW[me];
			list = AddCaptureP(list, IPawn[me], from, to, 0, bonus(to));
		}
		for (v = KAtt[kMe] & Current->mask & (~Current->att[opp]); T(v); Cut(v))
		{
			int from = kMe, to = lsb(v);
			list = AddCaptureP(list, IKing[me], from, to, 0, bonus(to));
		}
		for (u = Knight(me); T(u); Cut(u))
		{
			int from = lsb(u);
			for (v = NAtt[from] & Current->mask; T(v); Cut(v))
			{
				int to = lsb(v);
				list = AddCaptureP(list, IKnight[me], from, to, 0, bonus(to));
			}
		}
		for (u = Bishop(me); T(u); Cut(u))
		{
			int from = lsb(u), mover = PieceAt(from);
			for (v = BishopAttacks(from, PieceAll()) & Current->mask; T(v); Cut(v))
			{
				int to = lsb(v);
				list = AddCaptureP(list, mover, from, to, 0, bonus(to));
			}
		}
		for (u = Rook(me); T(u); Cut(u))
		{
			int from = lsb(u);
			for (v = RookAttacks(from, PieceAll()) & Current->mask; T(v); Cut(v))
			{
				int to = lsb(v);
				list = AddCaptureP(list, IRook[me], from, to, 0, bonus(to));
			}
		}
		for (u = Queen(me); T(u); Cut(u))
		{
			int from = lsb(u);
			for (v = QueenAttacks(from, PieceAll()) & Current->mask; T(v); Cut(v))
			{
				int to = lsb(v);
				list = AddCaptureP(list, IQueen[me], from, to, 0, bonus(to));
			}
		}
	}
...
template<bool me> int* gen_quiet_moves(int* list)
{
	...
	auto xray = [&](int loc) { return T(Current->xray[opp]) && F(Current->xray[opp] & FullLine[kLoc][loc]) ? FlagCastling : 0; };
...
pair<bool, vector<string>> parse_game(const std::string& line, std::unordered_set<uint64>& known_hashes)
{
	...
	constexpr double NO_VAL = -111.11;
	...
	try
	{
		...
		for (size_t start = 0, plies = 0, stop; ++plies <= 200; start = stop)	// stop after 100 moves
		{
			...
			if (e_loc == std::string::npos)
				break;
			stop = line.find('}', e_loc) + 1;
			size_t e_end = line.find(']', e_loc);
			string e_str(line.begin() + e_loc + 5, line.begin() + e_end);
			if (e_str.find('#') != string::npos)
				break;	// eval is mate
			double e_val = std::stod(e_str);
			if (pvs_val != NO_VAL && abs(e_val - pvs_val) > 2.5)
				break;	// patzer alert
			pvs_val = e_val;
			se2 += Square(e_val);
			if (se2 > 200.0)	// ignore long blowouts
				break;
			// now parse the move
			size_t m_loc = line.find_first_not_of("0123456789. ", start + 2);
			int move = parse_move(line.begin() + m_loc);
			if (move == MOVE_AMBIGUOUS)
			{
				std::cerr << "Ambiguous move at " << line.substr(start, stop - start) << "\n";
				break;
			}
				...
				else
				{
					...
					else
					{
						for (const auto& entry : score.why->vals)
						{
							...
							if (!contrib[0] && !contrib[1] && !contrib[2] && !contrib[3])
								continue;
							say("+ " + to_string(entry.line));
							for (auto c : contrib)
								say(" " + to_string(c) + " ");
							say(to_string(entry.src));
							if (entry.info != NO_INFO)
								say(" i " + to_string(entry.info));
							say("\n");
						}
					}
		...
		std::cerr << " wrote " << n_evals << " records";
	}
	catch (...)
	{
		std::cerr << "Error\n";
...
template<class T_> void spit(const char* what, T_ vals)
{
	cout << what << "\n";
	for (int r = 2; r < 7; ++r)
	{
		cout << Opening(vals[r]) << " " << Endgame(vals[r]) << "\n";
	}
	cout << "\n";
}
...
void main(int argc, char* argv[])
{
	...
	init_eval();
	Console = true;
	spit("General", PasserGeneral);
	spit("Free", PasserFree);
	spit("Blocked", PasserBlocked);
	spit("Candidate", PasserCandidate);
	spit("Outside", PasserOutside);
	cin.ignore();
...
#undef LOCK
#undef UNLOCK
#include "src/tbconfig.h"
#include "src/tbcore.h"
#include "src/tbprobe.c"
#pragma warning(pop)
#pragma optimize("", on)
